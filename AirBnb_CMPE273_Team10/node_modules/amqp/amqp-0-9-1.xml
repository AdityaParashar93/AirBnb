	...
	<class name="basic" handler="channel" index="60"
		...
		<method name="ack" index="80" label="acknowledge one or more messages">
			<doc>
				...
				or Get-Ok
				methods. The client can ask to confirm a single message or a set of messages
				up to
			...
			<field name="multiple" domain="bit" label="acknowledge multiple messages">
				<doc>
					...
					that the
					client can acknowledge multiple messages with a single method. If set to
					zero,
					the delivery tag refers to a single message. If the multiple field is
					1, and the
					delivery tag is zero, tells the server to acknowledge all outstanding
					messages.
				</doc>
				<rule name="exists" on-failure="precondition-failed">
					<doc>
						The server MUST validate that a non-zero delivery-tag refers to a
						delivered
						message, and raise a channel exception if this is not the case. On a
						transacted
						channel, this check MUST be done immediately and not delayed until a
						Tx.Commit.
